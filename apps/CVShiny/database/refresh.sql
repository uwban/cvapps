...
CREATE SCHEMA IF NOT EXISTS date_refresh;

CREATE TABLE IF NOT EXISTS date_refresh.history(
datintreceived date,
schema_name text,
meddra_version text);
...
CREATE TABLE current2.reports_table AS SELECT * FROM remote.reports;
...
ALTER TABLE current2.reports_table ADD COLUMN age_y_clean numeric(6,3);

UPDATE current2.reports_table SET 
  age_y_clean = CASE 
    WHEN age_y IS NOT NULL THEN age_y
    WHEN age_y IS NULL AND age_unit_eng = 'Decade' THEN age * 10 +5
    WHEN age_y IS NULL AND age_unit_eng = 'Years' THEN age
    WHEN age_y IS NULL AND age_unit_eng = 'Months' THEN  age / 12
    WHEN age_y IS NULL AND age_unit_eng = 'Minutes' THEN 0
    WHEN age_y IS NULL AND age_unit_eng = 'Weeks' THEN age / 52
    WHEN age_y IS NULL AND age_unit_eng = 'Hours' THEN 0
    ELSE NULL
  END;
...
CREATE INDEX ON current2.reports_table (age_y_clean);

CREATE INDEX ON current2.reactions (meddra_version);
