...
add_term <- function(uri) {
  ...
  terms <- c(paste0(uri,"+AND+!death:1+AND+seriousness:Yes"), paste0(uri, "+AND+death:true"),  paste0(uri, "+AND+seriousness:No"))
  return (terms)
...
drugs_per_report <- function(uri) {
  ...
  response <- parse_response(paste0(uri, '&limit=1000000'))
