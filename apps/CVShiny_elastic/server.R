...
shinyServer(function(input, output, session) {
  ...
  pretty_json<-reactive({
    search_uri<-current_search$uri
    query<-paste0('{"query":{"query_string":{"query":"',search_uri,'"}}}')
    json<-Search(index='drug_event',body=query,raw=T,size=10000)%>%prettify()
    
    return(json)
  })
  
  
   output$json_obj<-downloadHandler(
     filename = function() {
       ...
       paste0(input$search_dataset_type, '_', current_drug, '_', current_rxn, '.json')
     },
     content = function(file){
      writeLines(pretty_json(),file)
     }
   )
  ...
  reportertable <- reactive({
    ...
    reporter_count <- counter(current_search$uri, 'reporter_type.keyword')
  ...
  seriousplot_data <- reactive({
    ...
    death_count <- counter(search_uri, 'outcome.keyword')
    death_count <- death_count[(death_count$category =="Death"),]
    seriousness_count <- counter(search_uri, 'seriousness.keyword')
  ...
  sexplot_data <- reactive({
    ...
    sex_data <- counter(current_search$uri, 'patient_gender.keyword')
  ...
  agegroup_data <-reactive({
    ...
    age_count <- counter(current_search$uri, 'patient_age_y',limit=200) %>%
  ...
  indication_data <- reactive({
    ...
    data <- counter(current_search$uri,'report_indication_eng.keyword',limit=20)
    data
  })
  ...
  suspect_data <- reactive({    
    ...
    data <- counter(current_search$uri, 'report_drugname_suspect.keyword',limit=20)
  ...
  concomitant_data <- reactive({
    ...
    data <- counter(current_search$uri, 'report_drugname_concomitant.keyword',limit=20)
    data
  })
...
#There is a small chance that just adding the top 1000 from both might not give the correct top 20 (if one appears high on suspect, but not in top 1000 of concomitant for instance),
  ...
  drugcount_data <- reactive({
    data <- counter(current_search$uri, 'n_drugs_per_report',limit=20)
    data
  })
  ...
  top_pt_data <- reactive({
    ...
    data <- counter(current_search$uri, 'reaction_pt.keyword',limit=20)
    data
  })
  ...
  top_hlt_data <- reactive({
    data <- counter(current_search$uri, 'reaction_pt.keyword',limit=50)
  ...
  outcomeplot_data <- reactive({
    ...
    search_uri<-current_search$uri
    outcome_count <- counter(search_uri, 'outcome.keyword')
...
############# Download Tab
  ...
  cv_download_reports <- reactive({
    ...
    if(input$search_dataset_type == "Report Data"){
       ...
       report_data<-do.call(bind_rows,report_data$report_drug_detail)
