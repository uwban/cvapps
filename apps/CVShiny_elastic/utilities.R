...
request <- function(search_uri) {
  ...
  r <-Search(index='drug_event',body=query,raw=T)%>%fromJSON()
  total <- r$hits$total
...
#Params:
#Return:
counter <- function(uri, count_term,limit=10){
search_uri <- paste0(
'{"query": {"query_string": {"query":"',uri,'"}},
  "aggs":{"count":{"terms":{"field":"',count_term,'","size":',limit,'}}}}')
  
  r<-Search(index='drug_event',body=search_uri,size=0,raw=T)%>%fromJSON()
  
  df<-r$aggregations$count$buckets
  colnames(df)<-c('category','doc_count')
  return(df)
}
...
create_uri <- function(startDate, endDate, gender='All', age=c(0, 125), rxn=NULL, soc=NULL, drug_inv='Any', drugname=NULL, seriousness=NULL, search_type='',exact='exact', ...) {
  ...
  search_uri <- paste0('datintreceived:[', toString(startDate), ' TO ', toString(endDate), ']')
  

  if(age[1] != 0 | age[2] != 125) {
    search_uri <- paste0(search_uri, ' AND patient_age_y:', '(>=', age[1], ' AND <=', age[2], ')')
  }
